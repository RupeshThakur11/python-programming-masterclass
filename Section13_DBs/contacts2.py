#!/usr/bin/env python3

"""
Example for placement holders when executing sql statements
"""

import sqlite3

db = sqlite3.connect("contacts.sqlite")
new_email = "update@update.com"
phone_num = input("Input phone number ")

# update_sql = "UPDATE contacts SET email = 'update@update.com' WHERE email = 1234"
update_sql = "UPDATE contacts SET email = ? WHERE phone = ?"  # placement holder helps against sql injections
print(update_sql)

update_cursor = db.cursor()
update_cursor.execute(update_sql, (new_email, phone_num))  # can execute only 1 statement at a time, executescript can do multiple
print("{} rows updated".format(update_cursor.rowcount))

print()
print("Are connections the same: {}".format(update_cursor.connection == db))
print()

update_cursor.connection.commit()  # committing using cursor (vs db.commit()) is better in larger programs
update_cursor.close()

for name, phone, email in db.execute("SELECT * FROM contacts"):
    print(name)
    print(phone)
    print(email)
    print("-" * 20)

db.close()
